...
def trainScaler(dane):
	...
	scaler.fit(X.values)
	return scaler
def trainScalerHPRegression(dane):
	X = dane.copy()[["Attack", "Defense", "Sp. Atk", "Sp. Def", "Speed", "Type 1", "Type 2", "Legendary"]]
	X = encodeTypes(X)
...
def prepareDatasetHPRegression(dane, scaler):
	X = dane.copy()[["Attack", "Defense", "Sp. Atk", "Sp. Def", "Speed", "Type 1", "Type 2", "Legendary"]]
	Y = dane.copy()[["HP"]]
	X = encodeTypes(X)
	x_scaled = scaler.transform(X.values)
	X = pd.DataFrame(x_scaled)
	X.columns = ["Attack", "Defense", "Sp. Atk", "Sp. Def", "Speed", "Type 1", "Type 2", "Legendary"]
	return X, Y
def prepareDatasetTotalRegression(dane, scaler):
	X = dane.copy()[["HP", "Attack", "Defense", "Sp. Atk", "Sp. Def", "Speed", "Type 1", "Type 2", "Legendary"]]
	if "Total" in dane.columns:
		Y = dane.copy()[["Total"]]
	else:
		Y = pd.DataFrame()
		Y["Total"] = dane[["HP", "Attack", "Defense", "Sp. Atk", "Sp. Def", "Speed"]].sum(axis=1)
	X = encodeTypes(X)
	x_scaled = scaler.transform(X.values)
	X = pd.DataFrame(x_scaled)
	X.columns = ["HP", "Attack", "Defense", "Sp. Atk", "Sp. Def", "Speed", "Type 1", "Type 2", "Legendary"]
